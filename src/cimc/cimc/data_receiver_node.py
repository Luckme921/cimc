...
class AbbTaskTimer:
    """Thread-safe end-to-end timer for one in-flight ABB capture task."""
    ...
    def __init__(self):
        ...
        self._success_total_ms = 0.0
        self._success_min_ms = None
        self._success_max_ms = None

    def start(self, started_ns):
        with self._lock:
    ...
    def finish(self, finished_ns, success):
        with self._lock:
            ...
            self._active = None
            elapsed_ms = (
                int(finished_ns) - record['started_monotonic_ns']) / 1.0e6
            record['finished_monotonic_ns'] = int(finished_ns)
            record['elapsed_ms'] = elapsed_ms

            if success:
                self._success_count += 1
                self._success_total_ms += elapsed_ms
                if self._success_min_ms is None:
                    self._success_min_ms = elapsed_ms
                    self._success_max_ms = elapsed_ms
                else:
                    self._success_min_ms = min(
                        self._success_min_ms, elapsed_ms)
                    self._success_max_ms = max(
                        self._success_max_ms, elapsed_ms)

            record['success_count'] = self._success_count
            record['average_ms'] = (
                self._success_total_ms / self._success_count
                if self._success_count else None)
            record['minimum_ms'] = self._success_min_ms
            record['maximum_ms'] = self._success_max_ms
            return record
...
class DataReceiverNode(Node):
    def __init__(self):
        ...
        self.abb_task_timer = AbbTaskTimer()
        ...
        self.abb_capture_command = self.declare_parameter(
        ...
        if (not self.host or not self.target_ip or not self.forward_ip or
                not self.abb_capture_command):
            raise ValueError(
                'listen/ABB/forward IP and capture command must be non-empty')
        ...
        self.abb_task_timing_pub = self.create_publisher(
            String, '/abb/task_timing', 10)
        self.abb_tx_sub = self.create_subscription(
            String, '/abb/tx_text', self.abb_tx_callback, 10)
        self.weld_task_status_sub = self.create_subscription(
            String, '/weld_task/status', self.weld_task_status_callback, 10)
    ...
    def abb_tx_callback(self, msg):
        ...
        if not msg.data:
            return
        is_trajectory = msg.data.startswith('TRAJECTORY_BEGIN:')
        try:
            ...
            if is_trajectory:
                self.finish_abb_task_timing(
                    False, 'trajectory is not ASCII and was not sent')
        except queue.Full:
            self.publish_abb_tx_status(False, 'ABB transmit queue is full')
            if is_trajectory:
                self.finish_abb_task_timing(
                    False, 'ABB transmit queue is full')
    ...
    def weld_task_status_callback(self, msg):
        """Attach the coordinator task id and close failed task timers."""
        try:
    ...
    def start_abb_task_timing(self):
        ...
        if record is None:
            self.get_logger().warn(
                '[ABB端到端耗时] 已有任务计时中，'
                '新START_CAPTURE不会重置起点')
            return
        self.get_logger().info(
            f"[ABB端到端耗时] sequence={record['sequence']} 开始："
            '已收到完整START_CAPTURE')

    def finish_abb_task_timing(self, success, message, bytes_sent=0):
        record = self.abb_task_timer.finish(
            time.perf_counter_ns(), success)
        if record is None:
            return None
        body = {
            ...
            'start_event': 'ABB START_CAPTURE received',
            'end_event': (
                'trajectory sendall completed' if success else 'task failed'),
            'message': message,
            'success_count': record['success_count'],
            'average_ms': (
                round(record['average_ms'], 3)
                if record['average_ms'] is not None else None),
            'minimum_ms': (
                round(record['minimum_ms'], 3)
                if record['minimum_ms'] is not None else None),
            'maximum_ms': (
                round(record['maximum_ms'], 3)
                if record['maximum_ms'] is not None else None),
        }
        timing = String()
        timing.data = json.dumps(
            body, ensure_ascii=False, separators=(',', ':'))
        self.abb_task_timing_pub.publish(timing)

        task_label = (
            str(record['task_id'])
            if record['task_id'] is not None else 'unknown')
        if success:
            self.get_logger().info(
                f"[ABB端到端耗时] task_id={task_label}, "
                f"本次={body['elapsed_ms']:.3f} ms, "
                f"成功样本={body['success_count']}, "
                f"平均={body['average_ms']:.3f} ms, "
                f"最小={body['minimum_ms']:.3f} ms, "
                f"最大={body['maximum_ms']:.3f} ms, "
                f"发送={bytes_sent} bytes")
        else:
            self.get_logger().error(
                f"[ABB端到端耗时] task_id={task_label}, "
                f"失败耗时={body['elapsed_ms']:.3f} ms: {message}")
        return body
    ...
    def abb_sender_loop(self):
        ...
        while rclpy.ok() and self.is_running:
            ...
            is_trajectory = payload.startswith(b'TRAJECTORY_BEGIN:')
            if connection is None:
                self.publish_abb_tx_status(
                    False, 'ABB is disconnected; outbound message was dropped')
                if is_trajectory:
                    self.finish_abb_task_timing(
                        False, 'ABB disconnected; trajectory was dropped')
                continue
            try:
                connection.sendall(payload)
                timing = None
                if is_trajectory:
                    timing = self.finish_abb_task_timing(
                        True, 'trajectory sent to ABB socket', len(payload))
                status_text = f'sent {len(payload)} bytes to ABB'
                if timing is not None:
                    status_text += (
                        f"; task_elapsed_ms={timing['elapsed_ms']:.3f}")
                self.publish_abb_tx_status(True, status_text)
            except (OSError, socket.error) as exc:
                self.publish_abb_tx_status(False, f'ABB send failed: {exc}')
                if is_trajectory:
                    self.finish_abb_task_timing(
                        False, f'ABB send failed: {exc}')
    ...
    def process_received_text(self, text_data):
        ...
        commands = [line.strip() for line in clean_text.split('\n')]
        if any(
                line == self.abb_capture_command or
                line.startswith(self.abb_capture_command + ':')
                for line in commands):
            self.start_abb_task_timing()
                    ...
                    if client_ip == self.target_ip:
                        ...
                        while rclpy.ok() and self.is_running:
                            try:
                                ...
                                break
                        self.finish_abb_task_timing(
                            False, 'ABB disconnected before trajectory send')
                    else:
