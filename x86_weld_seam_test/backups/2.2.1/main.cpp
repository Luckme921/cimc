    ...
    const Eigen::Vector3f& n_bottom)
{
    ...
    while (group_begin < corners.size()) {
        ...
        while (group_end < corners.size()) {
            ...
            if (corners[group_end].protruding != corners[group_begin].protruding ||
        ...
        const int detection_count = static_cast<int>(group_end - group_begin);
        if (detection_count == 1) {
            merged_corners.push_back(corners[group_begin]);
            group_begin = group_end;
            continue;
        }

        WeldFeaturePoint merged = corners[group_begin];
        merged.merged_detection_count = detection_count;
        Eigen::Vector3f sum_ideal = Eigen::Vector3f::Zero();
        float sum_left_slope = 0.0f;
        float sum_right_slope = 0.0f;
        float minimum_ideal_z = std::numeric_limits<float>::max();
        for (size_t i = group_begin; i < group_end; ++i) {
            sum_ideal += corners[i].ideal_local;
            sum_left_slope += corners[i].left_line_slope;
            sum_right_slope += corners[i].right_line_slope;
            minimum_ideal_z = std::min(minimum_ideal_z, corners[i].ideal_local.z());
        }

        if (merged.protruding) {
            // 凸角均为直线理论交点。取组内中心估计而不保留某个极端交点，
            // 可降低短伪线段造成的过冲，同时仍位于圆角外侧的理论折角附近。
            Eigen::Vector3f sum_world = Eigen::Vector3f::Zero();
            Eigen::Vector3f sum_local = Eigen::Vector3f::Zero();
            float sum_distance = 0.0f;
            float minimum_local_z = std::numeric_limits<float>::max();
            for (size_t i = group_begin; i < group_end; ++i) {
                sum_world += corners[i].world;
                sum_local += corners[i].local;
                sum_distance += corners[i].distance_to_ideal;
                minimum_local_z = std::min(minimum_local_z, corners[i].local.z());
            }
            const float inverse_count = 1.0f / static_cast<float>(detection_count);
            merged.world = sum_world * inverse_count;
            merged.local = sum_local * inverse_count;
            merged.distance_to_ideal = sum_distance * inverse_count;
            // XY 使用中心估计，Z 使用组内更靠近底面的稳健候选高度。
            merged.world += (minimum_local_z - merged.local.z()) * n_bottom;
            merged.local.z() = minimum_local_z;
        } else {
            // 重复凹角此处只合并理论几何；安全圆弧点在后续单点构建阶段统一选择
            // 以支撑点弦中点重新搜索，避免某个旧候选背面点影响最终结果。
            Eigen::Vector3f sum_world = Eigen::Vector3f::Zero();
            Eigen::Vector3f sum_local = Eigen::Vector3f::Zero();
            for (size_t i = group_begin; i < group_end; ++i) {
                sum_world += corners[i].world;
        ...
        const float inverse_count = 1.0f / static_cast<float>(detection_count);
        merged.ideal_local = sum_ideal * inverse_count;
        merged.ideal_local.z() = minimum_ideal_z;
        merged.left_line_slope = sum_left_slope * inverse_count;
        merged.right_line_slope = sum_right_slope * inverse_count;
        merged.merged_detection_count = detection_count;

        std::cout << "Merged " << detection_count << " nearby "
            << (merged.protruding ? "protruding" : "recessed")
            << " detections into one feature at local_x=" << merged.ideal_local.x()
            << std::endl;
    ...
    const FeatureExtractionParams& params)
{
    ...
    // 只按顺序取有效直线段。中间若夹着圆角、短噪声段或孔洞形成的无效段，
    // 仍然允许前后两条真实直线求交，这是上一版漏掉部分“直线起点”的主因。
    ...
    for (size_t valid_i = 0; valid_i + 1 < valid_run_indices.size(); ++valid_i) {
        ...
        const SeamLineRun& right =
            runs[static_cast<size_t>(valid_run_indices[valid_i + 1])];

        const float observed_gap = right.x_first - left.x_last;
        if (observed_gap < 0.0f || observed_gap > params.max_hole_bridge) continue;
        ...
        const float ideal_x = (right.line.intercept - left.line.intercept) / denominator;
        const float ideal_y = left.line.slope * ideal_x + left.line.intercept;

        // 拍摄边界之外不制造角点；边界残段本身不再作为焊接特征输出。
        if (ideal_x < bins.front().local_x || ideal_x > bins.back().local_x) continue;

        // 交点应在两段端点之间（可额外外推少量距离以覆盖圆角与孔洞）。
        ...
        } else if (right.type == SEGMENT_FLAT) {
...
#undef APPLY_FLOAT
    ...
    for (size_t i = 0; i < feature_points.size(); ++i) {
        ...
        if (feature.is_transition_point) {
            ...
            if (feature.protruding) source_name = "ideal intersection";
